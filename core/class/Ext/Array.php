...
class Ext_Array {
    ...
    public static function map($array, $fun){
        foreach($array as $key=>$item){
            $array[$key] = is_array($item) ? self::map($item,$fun):call_user_func(
                    $fun,$item);
        }
        return $array;
    }
    ...
    public static function sort($array, $keyFields = 0, $sortTypes = 'asc'){
        ...
        foreach($valueArray as $key=>$item){
            if (!is_null($lastItem) &&
                     $array[$key][$keyField] != $lastItem[$keyField]) $i++;
     ...
     * 序列转换数组
     *
     * @param string $strSerial
     *            来源序列
     * @param string $strSplitMain
     *            序列一级分隔符
     * @param string $strSplitSub
     *            序列二级分隔符
     * @return array
     */
    public static function serialToArray($strSerial, $strSplitMain = '|', 
            $strSplitSub = ':'){
        $arrResult = array();
        if ($strSerial){
     ...
     * @return string
     */
    public static function arrayToSerial($array, $strSplitMain = '|', 
            $strSplitSub = ':'){
        foreach($array as $key=>$item){
