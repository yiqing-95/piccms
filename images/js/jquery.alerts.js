...
(function($) {
	...
	$.alerts = {
		...
		overlayOpacity: 0.75,                // transparency level of overlay
		...
		success: function(message, title, callback) {
			if( title == null ) title = '操作成功';
			$.alerts._show(title, message, null, 'success', function(result) {
				if( callback ) callback(result);
			});
		},
		
		window: function(message, title, callback) {
			if( title == null ) title = '弹出框';
		...
		_show: function(title, msg, value, type, callback) {
			...
			var html = '<div id="popup_container">' +
			    '<div id="popup_nav"><span id="popup_title"></span><span id="popup_close"><a href="javascript:$.alerts._hide();">&nbsp;</a></span></div>' +
			    '<div id="popup_content">';
			if (type == 'box') {
				html += '<div id="popup_message" style="width:500px; height:300px; overflow:auto"></div>';
			} else {
			...
			$("#popup_title").width($("#popup_container").width() - 60);
			// Make draggable
			if( $.alerts.draggable ) {
				try {
